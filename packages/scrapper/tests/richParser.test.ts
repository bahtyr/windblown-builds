...
const FIREWORKS_ITALIC_NEWLINE = `<td style=""><span>Its explosion lightens and <b>stuns nearby enemies</b><br> <i>MY EYES!</i></span> </td>`
...
describe("rich text parser", () => {
  ...
  it("parse full gift: Cursed Criticals", () => {
    ...
    expect(asText).not.toContain("Alterattacks");
    expect(asText).not.toContain("Final Curse");
    expect(asText).not.toContain("Each stack increases damage");
  });

  it("parse trinket: Fireworks", () => {
    const tokens = parse(FIREWORKS_ITALIC_NEWLINE);
    expect(tokens).toEqual([
      {
        "key": "text",
        "text": "Its explosion lightens and ",
      },
      {
        "bold": true,
        "key": "text",
